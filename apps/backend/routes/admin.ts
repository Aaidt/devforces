...
adminRouter.post("/hiring/profile_pic/url", userMiddleware, async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Server error: Failed to get presigned url" });
    }
  },
);
...
adminRouter.post("/hiring/details/confirm", userMiddleware, async (req, res) => {
    ...
    if (!key) {
      res
        .status(404)
        .json({ message: "No profile pic found. Upload the image first." });
      return;
    }
    ...
    try {
      ...
      await prismaClient.recruiter.update({
      ...
      res
        .status(500)
        .json({ message: "Server error: Failed to update company details" });
    }
  },
);
...
adminRouter.post("/hiring/post", adminMiddleware, async (req, res) => {
  ...
  try {
    const result = await prismaClient.$transaction(async (tx) => {
      ...
      if (questionConfig && questionConfig.questionTypes.length > 0) {
        ...
        for (let i = 0; i < generatedQuestions.length; i++) {
          ...
          await tx.contest_to_Challenge_mapping.create({
            data: {
              contest_id: contest.id,
    ...
    res
      .status(500)
      .json({ message: "Server error: Failed to create hiring post" });
  }
});
