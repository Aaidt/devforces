          ...
          <div className="flex-1 space-y-4 pt-2">
            <div>
              <div className="flex items-center justify-center md:justify-start gap-3 flex-wrap">
                ...
                <span className={`text-xs font-semibold px-3 py-1 rounded-full ${isAdmin ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-blue-500/10 text-blue-400 border border-blue-500/20"}`}>
                  {isAdmin ? "Hiring" : "Candidate"}
                </span>
              </div>
            ...
            {isAdmin && profile.user.company_name && (
              <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-4 space-y-1">
                ...
                {profile.user.company_description && (
                  <p className="text-zinc-400 text-sm">{profile.user.company_description}</p>
                )}
                <div className="flex items-center gap-4 text-xs text-zinc-500 pt-1">
                  {profile.user.company_website && (
                    <a href={profile.user.company_website} target="_blank" className="hover:text-emerald-400 transition-colors">
                      {profile.user.company_website}
                    </a>
                  )}
                  {profile.user.company_employees && (
                    <span>{profile.user.company_employees} employees</span>
                  )}
                </div>
              </div>
            )}
